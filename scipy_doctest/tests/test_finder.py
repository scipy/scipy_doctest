...
from ..impl import DTConfig, DTFinder
from ..util import get_all_list, get_public_objects
from . import finder_cases, finder_cases_2
...
class TestSkiplist:
    ...
    def test_get_doctests_strategy_list(self):
        ...
        tests = find_doctests(
            finder_cases, strategy=[finder_cases.Klass], config=config
        )
        names = [t.name for t in tests]
